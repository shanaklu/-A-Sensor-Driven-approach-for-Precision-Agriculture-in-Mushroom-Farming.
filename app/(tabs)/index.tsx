...
const HomeScreen = () => {
    ...
    const [humanPresence, setHumanPresence] = useState<boolean | null>(true);
    const [soilMoisture, setSoilMoisture] = useState<number | null>(68);
    const [humidity, setHumidity] = useState<number | null>(45);
    const [tempHistory, setTempHistory] = useState<number[]>([22, 23, 24, 25, 26]);
    ...
    const fetchData = () => {
        setLoading(true);
        setError(null);
        const tempRef = firebase.database().ref('temperature');
        const presenceRef = firebase.database().ref('humanPresence');
        const soilRef = firebase.database().ref('soilMoisture');
        ...
        const onTempChange = tempRef.on('value', snapshot => {
            if (snapshot.val()) {
                ...
                setTempHistory(prev => [...prev.slice(-5), value]);
            }
            setLoading(false);
        }, (err: Error) => {
        ...
        const onPresenceChange = presenceRef.on('value', snapshot => {
            if (snapshot.val()) {
        ...
        const onSoilChange = soilRef.on('value', snapshot => {
            if (snapshot.val()) {
        ...
        const onHumidityChange = humidityRef.on('value', snapshot => {
            if (snapshot.val()) {
                const { value } = snapshot.val();
                setHumidity(value);
            }
        ...
        return () => {
            ...
            presenceRef.off('value', onPresenceChange);
            soilRef.off('value', onSoilChange);
            humidityRef.off('value', onHumidityChange);
        };
    };
